...
class Table_DES
{
    ...
    int DES_PC1_Transform(type key[64], type tempbts[56])
    {
        ...
        {
            tempbts[cnt] = key[PC_1[cnt]]; //根据PC_1矩阵置换
        }
        return 0;
    }
    //密钥置换pc2
    int DES_PC2_Transform(type key[56], type tempbts[48])
    {
        ...
        {
            tempbts[cnt] = key[PC_2[cnt]]; //根据PC_2矩阵置换
        }
        return 0;
    }
    ...
    int DES_SBOX(type data[48])
    {
        ...
        for (cnt = 0; cnt < 8; cnt++)
        {
            ...
            // S盒6位输入的第二至五位构成一个四位的二进制数，对应于s盒中的某一列。
    ...
    //加密文件
public:
    int treble_DES_Encrypt(char *plainFile, type *key_1, type *key_2, type *key_3, char *cipherFile)
    {
        FILE *plain, *cipher;
        int count;
        long fileLen;
        ...
        //通过判断 fopen() 的返回值是否和 NULL 相等来判断是否读写失败
    ...
    //解密文件
public:
    int treble_DES_Decrypt(char *cipherFile, type *key_1, type *key_2, type *key_3, char *plainFile)
    {
        FILE *plain, *cipher;
        int count, times = 0;
        long fileLen;
